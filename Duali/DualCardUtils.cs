...
namespace Duali
{
    public class DualCardUtils
    {
        ...
        public TransparentResponse Read(int fileNumber, byte flag, int offset, int length)
        {
        ...
        public TransparentResponse Write(int fileNumber, byte flag, int offset, string data)
        {
        ...
        public TransparentResponse ChangeKey(int keyNumber, int keyVersion, string oldKey, string newKey)
        {
            ...
            return Transparent(changeKeyCommand, DataUtils.DecToHex(keyVersion) + DataUtils.DecToHex(oldKey.Length / 2) + DataUtils.DecToHex(newKey.Length / 2) + oldKey + newKey);
        }
        ...
        public byte ComputeFlag(bool cmac, bool af, bool crc32, bool crc16, bool encipher)
        {
            String flag = "000";

            if (cmac) { flag += "1"; } else { flag += "0"; }
            if (af) { flag += "1"; } else { flag += "0"; }
            if (crc32) { flag += "1"; } else { flag += "0"; }
            if (crc16) { flag += "1"; } else { flag += "0"; }
            if (encipher) { flag += "1"; } else { flag += "0"; }

            return DataUtils.BinaryStringToByte(flag);
        }
        ...
        public void Anticol()
        {
            ...
            Duali.DualCard.DEA_Idle_Req(port, out outlen, lpRes);
            Duali.DualCard.DEA_AntiSelLevel(port, out outlen, lpRes);
        }
        ...
        public string APDU(String command)
        {
            ...
            int outlen;
            byte[] lpRes = new byte[256];
            byte[] commandByte = DataUtils.HexStringToBytes(command);
            int response = Duali.DualCard.DE_APDU(port, commandByte.Count(), commandByte, out outlen, lpRes);
            String data = "";
            if (response == 0)
            {
                data = DataUtils.HexToString(DataUtils.BytesToHex(lpRes));
                data = data.Substring(1, outlen - 1);
            }
        ...
        {
            byte[] keyInBytes = DataUtils.HexStringToBytes(key);
            byte keyTypeByte = keyType == KeyType.TYPE_A ? (byte)0x00 : (byte)0x04;
            return Duali.DualCard.DEA_Authkey(port, keyTypeByte, keyInBytes, Convert.ToByte(block)) == 0;
        }
        ...
        public String ReadMifare(int block)
        {
            int outlen;
            byte[] lpRes = new byte[256];
            int response = Duali.DualCard.DEA_Read(port, Convert.ToByte(block), out outlen, lpRes);
            String data = "";
            if (response == 0)
            {
                data = DataUtils.HexToString(DataUtils.BytesToHex(lpRes));
        ...
        public Boolean WriteMifare(string data, int block)
        {
            byte[] dataInBytes = DataUtils.HexStringToBytes(DataUtils.StringToHex(data));
            byte[] writeBytes = Enumerable.Repeat<byte>(32, 16).ToArray();
            Array.Copy(dataInBytes, 0, writeBytes, 0, dataInBytes.Length);
            return Duali.DualCard.DEA_Write(port, Convert.ToByte(block), 16, dataInBytes) == 0;
        }

        public Boolean WriteMifareHex(string data, int block)
        {
            byte[] dataInBytes = DataUtils.HexStringToBytes(data);
            byte[] writeBytes = Enumerable.Repeat<byte>(32, 16).ToArray();
            Array.Copy(dataInBytes, 0, writeBytes, 0, dataInBytes.Length);
            return Duali.DualCard.DEA_Write(port, Convert.ToByte(block), 16, dataInBytes) == 0;
        }
